...
$(function () {
  ...
  var lastModifyEntryKey = ''; // firebase
  ...
  $('#edit-button').on('click', function (e) {
    ...
    var data = getFormData();
    var dataRef; // パスワードの入力がなければ旧パスワードのまま
  ...
  $('#delete-button').on('click', function (e) {
    e.preventDefault();
    enableActionButtons(false); // validate
  ...
  function validate(isMediation, isEdit) {
    ...
    if (!isMediation) {
      for (var _i = 1; _i < 3; _i++) {
        ...
        if ($("#member".concat(num, "-name")).val() || $("#member".concat(num, "-character")).prop('selectedIndex') !== 0 || $("#member".concat(num, "-comment")).val() || $("#member".concat(num, "-region")).prop('selectedIndex') !== 0 || $("#member".concat(num, "-env")).prop('selectedIndex') !== 0 || $("#member".concat(num, "-history")).val() || $("#member".concat(num, "-join-ok")).prop('checked') || $("#member".concat(num, "-join-ng")).prop('checked')) {
          if ($("#member".concat(num, "-name")).val().length === 0) {
            showMessage('danger', "".concat(num, "\u4EBA\u76EE\u306E\u30D7\u30EC\u30A4\u30E4\u30FC\u540D\u3092\u5165\u529B\u3057\u3066\u304F\u3060\u3055\u3044"));
            return false;
          }

          if ($("#member".concat(num, "-character")).prop('selectedIndex') === 0) {
            showMessage('danger', "".concat(num, "\u4EBA\u76EE\u306E\u4F7F\u7528\u30AD\u30E3\u30E9\u3092\u9078\u629E\u3057\u3066\u304F\u3060\u3055\u3044"));
            return false;
          }
        }
      }
  ...
  function getFormData() {
    ...
    for (var _i2 = 0; _i2 < 3; _i2++) {
      ...
      if ($("#member".concat(num, "-name")).val()) {
        data.members.push({
          name: $("#member".concat(num, "-name")).val(),
          character: $("#member".concat(num, "-character")).val(),
          twitter: $("#member".concat(num, "-twitter")).val(),
          comment: $("#member".concat(num, "-comment")).val().replace(/[\n\r]/g, ''),
          region: $("#member".concat(num, "-region")).val(),
          env: $("#member".concat(num, "-env")).val() || '',
          history: $("#member".concat(num, "-history")).val() || '',
          after: $("input[name=member".concat(num, "-after]:checked")).val() || ''
        });
      }
    }
  ...
  function setFormData(entry, key) {
    ...
    for (var _i3 = 0; _i3 < entry.members.length; _i3++) {
      var num = _i3 + 1;
      $("#member".concat(num, "-name")).val(entry.members[_i3].name);
      $("#member".concat(num, "-character")).val(entry.members[_i3].character || '選択してください');
      $("#member".concat(num, "-twitter")).val(entry.members[_i3].twitter);
      $("#member".concat(num, "-comment")).val(entry.members[_i3].comment);
      $("#member".concat(num, "-region")).val(entry.members[_i3].region || '選択してください');
      $("#member".concat(num, "-env")).val(entry.members[_i3].env || '選択してください');
      $("#member".concat(num, "-history")).val(entry.members[_i3].history);
      var after = entry.members[_i3].after;

      if (after) {
        if (after == 1) {
          $("#member".concat(num, "-join-ok")).prop('checked', true);
        } else {
          $("#member".concat(num, "-join-ng")).prop('checked', true);
        }
      }
    }
  ...
  function showMessage(type, message, fadeOut) {
    $('#message').empty(); // if (typeof fadeOut === 'undefined') {
    //   fadeOut = false;
    // }

    $('#message')[0].className = '';
    $('#message').text(message).addClass('alert alert-' + type).show(); // if (fadeOut) {
    //   setTimeout(function(){
  ...
  function showMessage2(type, message, fadeOut) {
    $('#message2').empty(); // if (typeof fadeOut === 'undefined') {
    //   fadeOut = false;
    // }

    $('#message2')[0].className = '';
    $('#message2').text(message).addClass('alert alert-' + type).show(); // if (fadeOut) {
    //   setTimeout(function(){
  ...
  function scrollTo(selector, adjust) {
    $("html,body").animate({
      scrollTop: $(selector).eq(0).offset().top + adjust
    });
  }
  ...
  function analytics() {
    ...
    task = setTimeout(function () {
      ...
      displayCharaCount(pCharasArray, '#character-count'); // 都道府県別集計
