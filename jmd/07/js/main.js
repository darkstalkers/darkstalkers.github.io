...
(function () {
  ...
  $(document).on('click', '.gnav a', function (e) {
    e.preventDefault(); // var to = $($(this).attr('href')).position().top - 5;

    var to = $($(this).attr('href')).position().top + con.scrollTop() - 5; // console.log($(this).attr('href'), to);
  ...
  if (window.innerWidth > 600) {
    ...
    bg.addEventListener('scroll', function (e) {
      ...
      bg.style.backgroundPosition = "center calc(140px + ".concat(diff, "%)");
    }, false);
  } else {
  ...
  $(window).load(function (e) {
    $(".fancybox, .fancybox").fancybox(); // var isDisp = false,
    //     dispBtn = $('#disp');
